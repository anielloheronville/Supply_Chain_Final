...
st.set_page_config(layout="wide", page_title="Dashboard de Logística e Supply Chain")

# Título do Dashboard
st.title("🚢 Dashboard de Análise Logística e Supply Chain")
st.markdown("Uma análise interativa de dados simulados de logística de portos brasileiros.")

# --- 1. CARREGAMENTO E CACHE DOS DADOS ---
@st.cache_data
def load_data():
    try:
        # Use o nome do seu arquivo de dados (o completo ou a amostra)
        # Para o deploy, é recomendado usar a amostra: 'logistics_database_sample.csv'
        df = pd.read_csv(
            'logistics_database_sample.csv', 
            sep=';',
            decimal=',',
            parse_dates=['Data_Prometida', 'Data_Entrega_Real'],
            encoding='utf-8-sig'
        )

        # --- CORREÇÃO APLICADA ---
        # 1. Força a coluna 'Order Date' para datetime, transformando erros em NaT (Not a Time)
        df['Order Date'] = pd.to_datetime(df['Order Date'], errors='coerce')

        # 2. Remove as linhas onde 'Order Date' é NaT (ou seja, a data era inválida)
        df.dropna(subset=['Order Date'], inplace=True)
        # ---------------------------

        coordenadas_portos = {
            'Porto de Santos': (-23.988, -46.303), 'Porto de Paranaguá': (-25.505, -48.513),
            'Porto de Itaguaí': (-22.910, -43.818), 'Porto do Itaqui': (-2.587, -44.364),
            'Porto de Rio Grande': (-32.097, -52.122), 'Porto de Suape': (-8.397, -34.962),
            'Porto de São Francisco do Sul': (-26.244, -48.643), 'Porto de Vila do Conde': (-1.551, -48.775),
            'Porto de Santarém': (-2.422, -54.721), 'Porto do Rio de Janeiro': (-22.896, -43.176),
            'Porto Itapoá': (-26.115, -48.614), 'Portonave': (-26.885, -48.653),
            'Porto Chibatão': (-3.155, -59.972), 'Porto de Itajaí': (-26.908, -48.653),
            'Terminal Marítmo de Ponta da Madeira': (-2.529, -44.357), 'Terminal de Tubarão': (-20.285, -40.244),
            'Terminal Aquaviário de Angra dos Reis': (-23.018, -44.312), 'Porto de Manaus': (-3.136, -60.021),
            'Porto de Vitória': (-20.316, -40.338), 'Porto do Açu': (-21.841, -41.019),
            'Terminal Portuário do Pecém': (-3.535, -38.800), 'Porto de Salvador': (-12.962, -38.512),
        ...
        st.info("Certifique-se de que o arquivo 'logistics_database_sample.csv' está no seu repositório GitHub.")
        return None, None
...
all_carriers = sorted(df['Carrier'].unique())
...
all_cargo_types = sorted(df['Tipo_Carga'].unique())
...
# --- 3. KPIs PRINCIPAIS ---
st.header("KPIs de Desempenho Logístico")
...
if df_filtered.empty:
    ...
    with col2:
        ...
        st.metric(label="Taxa OTIF Média", value=f"{otif_rate:.2%}")
    with col3:
        avg_delay = df_filtered[df_filtered['Atraso_Total'] > 0]['Atraso_Total'].mean()
        st.metric(label="Atraso Médio (Dias)", value=f"{avg_delay:.2f}" if pd.notna(avg_delay) else "0.00")
    with col4:
...
st.header("Análise Geográfica Interativa")
tab1, tab2 = st.tabs(["🗺️ Mapa de Atividade dos Portos", "✈️ Mapa das Principais Rotas"])
...
with tab2:
    st.subheader("Análise das Top 25 Rotas por Volume de Pedidos")
        ...
        for _, rota in rotas_plot.iterrows():
            if pd.notna(rota['Origin_Coords']) and pd.notna(rota['Destination_Coords']):
                ...
                AntPath(
                    ...
                    tooltip=(f"<b>Rota:</b> {rota['Origin Port']} → {rota['Destination Port']}<br>"
                             f"<b>Volume:</b> {rota['Volume_Pedidos']} pedidos<br>"
                             f"<b>Atraso Médio:</b> {rota['Atraso_Medio_Total']:.2f} dias"),
...
st.header("Análises de Performance, Custos e Atrasos")
...
with col2:
    ...
    if not df_filtered.empty:
        ...
        report_costs['Rota'] = report_costs['Origin Port'] + ' → ' + report_costs['Destination Port']
        ...
        st.info("Nenhum dado para o gráfico de Top 10 Rotas por Custo.")
...
with col3:
    ...
    if not df_filtered.empty:
        ...
        st.info("Nenhum dado para o gráfico de OTIF por tipo de carga.")

with col4:
    st.subheader("Análise de Motivos de Atraso")
    if not df_filtered.empty and 'Atraso_Total' in df_filtered.columns:
        df_atrasados = df_filtered[df_filtered['Atraso_Total'] > 0]
        if not df_atrasados.empty:
            motivos = [
                'Congestionamento no Porto', 'Atraso na Alfândega', 'Problema Operacional',
                'Condições Climáticas', 'Falha de Equipamento', 'Inspeção da Carga', 'Greves'
            ]
            probabilidades_motivos = [0.25, 0.20, 0.15, 0.15, 0.10, 0.10, 0.05]
            
            # Usando uma cópia para evitar o SettingWithCopyWarning
            df_atrasados_copy = df_atrasados.copy()
            df_atrasados_copy['Motivo_Atraso'] = np.random.choice(motivos, size=len(df_atrasados_copy), p=probabilidades_motivos)

            report_delays = df_atrasados_copy['Motivo_Atraso'].value_counts().reset_index()
        ...
        st.info("Dados de atraso não disponíveis.")


# --- 6. ANÁLISE DE TENDÊNCIAS E CORRELAÇÃO ---
st.header("Análise de Tendências e Correlações")
tab3, tab4 = st.tabs(["📈 Tendência Mensal da Taxa OTIF", "🔗 Mapa de Calor de Correlação"])

with tab3:
    if not df_filtered.empty:
        report_trends = df_filtered.set_index('Order Date').groupby(pd.Grouper(freq='ME')).agg(OTIF_Rate=('OTIF', 'mean'))
        fig, ax = plt.subplots(figsize=(15, 7))
        report_trends['OTIF_Rate'].plot(kind='line', marker='o', linestyle='-', ax=ax)
        plt.title('Tendência Mensal da Taxa OTIF', fontsize=16)
        plt.ylabel('Taxa OTIF', fontsize=12)
        plt.xlabel('Mês', fontsize=12)
        plt.grid(True, which='both', linestyle='--', linewidth=0.5)
        ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: f'{y:.0%}'))
        ax.set_ylim(0, max(0.1, report_trends['OTIF_Rate'].max() * 1.2) if not report_trends.empty else 1)
        st.pyplot(fig)
    else:
        st.info("Nenhum dado para exibir o gráfico de tendências.")

with tab4:
    if not df_filtered.empty:
        # Selecionando apenas colunas numéricas para correlação
        numeric_cols = df_filtered.select_dtypes(include=np.number).columns.tolist()
        cols_for_corr = [col for col in ['Unit quantity', 'Weight', 'Custo_Total', 'Atraso_Total', 'Satisfacao_Cliente'] if col in numeric_cols]
        ...
        if len(cols_for_corr) > 1:
            ...
            st.info("Não há dados numéricos suficientes para gerar um mapa de calor.")
    else:
